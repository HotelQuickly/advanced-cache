...
class Base {
  ...
  lockAndLoad(resolve, reject, id, policy, loadFn, loadFnArgs) {
    const self = this

    this.redlock.lock(policy.lockKey, this.opts.lockIntervalMs, function handleLock(err, lock) {
      if (err) {
        debug('lockAndLoad', policy.key, id, `lock error. retry load in ${self.opts.retryIntervalMs}ms`)
        return Promise.delay(self.opts.retryIntervalMs)
          .then(() => self._getCached(resolve, reject, id, policy, loadFn, loadFnArgs))
      }

      debug('lockAndLoad', policy.key, id, 'successfully locked. loading with loadFn')

      loadFn.apply(null, loadFnArgs)
        .then(value => {
          debug('lockAndLoad', policy.key, id, 'loaded successfully')
          ...
          return setCachePromise
            .then(() => lock.unlock())
            .then(() => resolve(value))
            .then(() => debug('lockAndLoad', policy.key, id, 'set, unlocked'))
        })
  ...
  _getCached(resolve, reject, id, policy, loadFn, loadFnArgs) {
    ...
    this._getCache(policy)
      .then(value => {
        ...
        this.lockAndLoad(resolve, reject, id, policy, loadFn, loadFnArgs)
      })
  }
}
