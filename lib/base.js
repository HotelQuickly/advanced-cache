...
class Base {
  ...
  execute(policy, loadFn) {
    if(process.env.NODE_ENV === 'development' && process.env.ADVANCED_CACHE_BYPASS_CACHE === 'bypass') {
      return new Promise((resolve, reject) => this._bypassCache(resolve, loadFn))
    }
    const id = Math.ceil(Math.random() * 10000000)
    return new Promise((resolve, reject) => this._getCached(resolve, reject, id, policy, loadFn))
  }

  _bypassCache(resolve, loadFn) {
    loadFn()
      .then(value => {
