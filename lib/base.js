...
class Base {
  ...
  _getCached(resolve, reject, id, policy, loadFn) {
    debug('_getCached', policy.lockKey, id)

    this._getCache(policy)
      .then(value => {
        if (value) {
          debug('_getCached', policy.lockKey, id, 'found in cache. return')
          return resolve(value)
        }

        debug('_getCached', policy.lockKey, id, 'missing in cache. lock and load')
  ...
  _lockAndLoad(resolve, reject, id, policy, loadFn) {
    ...
    this.redlock.lock(policy.lockKey, this.opts.lockIntervalMs, function handleLock(err, lock) {
      if (err) {
        debug('_lockAndLoad', policy.lockKey, id, `lock error. retry load in ${self.opts.retryIntervalMs}ms`)
        return Promise.delay(self.opts.retryIntervalMs).then(() => self._getCached(resolve, reject, id, policy, loadFn))
      }

      debug('_lockAndLoad', policy.lockKey, id, 'successfully locked. loading with loadFn')
      ...
      loadFn()
        .then(value => {
          ...
          return setCachePromise
            .then(() => lock.unlock())
            .then(() => resolve(value))
            .then(() => debug('_lockAndLoad', policy.lockKey, id, 'unlocked. loaded successfully'))
        })
